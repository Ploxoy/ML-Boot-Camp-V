...
def clean_data(data, more_clean=False):
...
# 1.5 times faster than previous version.
# class FastLikelihood:
#     def __init__(self, columns, target_column, glob_mean_value, kf, alpha=13, seed=0, blur=None):
#         self.columns = columns
#         self.glob_mean_value = glob_mean_value
#         self.alpha = alpha
#         self.kf = kf
...
#             te = X.iloc[ite]
#
#             groups = tr.groupby(self.columns)
#             K = groups.size()
#             mean_loc = groups[self.target_column].mean()
#             values = (mean_loc * K + self.glob_mean_value * self.alpha) / (K + self.alpha)
#             values.name = self.new_column
#             result[ite] = te.join(values, on=self.columns)[self.new_column]
#
#         result = pd.DataFrame(result, columns=[self.new_column])
#         result[result.isnull()] = self.glob_mean_value
#
#         groups = X.groupby(self.columns)
#         K = groups.size()
#         mean_loc = groups[self.target_column].mean()
#         self.values = (mean_loc * K + self.glob_mean_value * self.alpha) / (K + self.alpha)
#         self.values.name = self.new_column
#
#         s = np.random.get_state()
#         np.random.set_state(self.rnd_state)
#
#         result += np.random.normal(scale=self.blur, size=result.shape)
#
#         self.rnd_state = np.random.get_state()
#         np.random.set_state(s)
#
...
#
#     def transform(self, X):
#         result = X.join(self.values, on=self.columns)[self.new_column]
#         result = pd.DataFrame(result, columns=[self.new_column])
#         result[result.isnull()] = self.glob_mean_value
#
#         s = np.random.get_state()
